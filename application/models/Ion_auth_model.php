...
class Ion_auth_model extends CI_Model
{
	...
	**/
	...
	public function set_session($user)
	{
		...
		$ident = $this->ion_auth->user()->row();
		
		$query = $this->db->select('m_prg_studi_id,urutan_verifikator,first_name')
		->where(array($this->identity_column => $user->{$this->identity_column},'active'=>'1'))
		->limit(1)
		->order_by('id', 'desc')
		->get($this->tables['users']);
		$m_prg_studi_id = $query->row()->m_prg_studi_id;
		$urutan_verifikator = $query->row()->urutan_verifikator;
		...
		$session_data = array(
			...
			//'last_name'       => $user->{$this->last_name},
			'm_prg_studi_id'       => $m_prg_studi_id,
			'urutan_verifikator'   => $urutan_verifikator,
			'last_check'           => time(),
			//'identitas_id'			=> '20208648872397',
			//'formulir_id'			=> 'SMB2'
